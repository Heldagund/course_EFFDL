...
class BC(nn.Module):
    ...
    def binarization(self):
        ...
        for index in range(self.num_of_params):
            # tmp = self.target_modules[index].data.clone()
            # ones = torch.ones_like(tmp)
            # self.target_modules[index].data.copy_(torch.where(tmp>=0, ones, torch.neg(ones)))
    ...
    def clip(self):
        ...
        ht = nn.Hardtanh(-1, 1)
        for index in range(self.num_of_params):
            self.target_modules[index].data.copy_(ht(self.target_modules[index].data))
...
def GenBCModel(modelBuilder):
    model = modelBuilder()
    BCModel = BC(model)
    print('Exit GenBCModel')
    return BCModel

def BCTrainRoutine(net, optimizer, criterion, inputs, labels):
    optimizer.zero_grad()
...
def BCTestRoutine(net, criterion, inputs, labels):
    net.binarization()
